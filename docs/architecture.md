...
- 속성: `confidence`, `provenance`, `valid_from`/`valid_to`(시간 유효구간).
...
| `provenance` | `host_id` · `workspace` · `source_ref`(파일/URL/툴) · `observed_at` · `confidence` |
...
### 2.4 출처(Provenance) — **1급 시민**
모든 사실은 출처 태그를 달고 저장된다. 어떤 것도 파괴적으로 덮어쓰지 않는다.
충돌은 모든 주장(assertion)을 provenance와 함께 보존하고, **해소 계층**이
"현재 믿음(current belief)"을 계산(예: (subject,predicate)별 최신 우선 또는 confidence 가중)한다.
...
- 이 복제 프리미티브는 경로(로컬·서버·피어)에 **무관** → §5의 모든 위상이 같은 로직 재사용.
...
| 벡터 검색 | ✅ 네이티브 HNSW | ❌ (별도 필요) |
| 그래프 순회 | ✅ 재귀 Datalog | ✅ SPARQL property path |
| 온톨로지 표준(OWL/RDFS) | 스키마를 직접 모델링 | ✅ 표준 최적 |
...
| 도구 | 역할 |
|------|------|
| `observe` | 지식 조각(+출처) 적재 → 관측 생성·엔티티 링크 |
...
- **출처·신뢰**: 모든 사실에 (host, workspace, source, confidence, time). 질의 시 출처 필터/가중.
- **시간성**: 관측은 타임스탬프, 관계는 유효구간 → "특정 시점에 우리가 알던 것" 시간여행 질의.
- **정체성 해소(entity resolution)**: 정규 키(정규화 명칭) 우선 + 임베딩 유사도 임계값. 병합 이력 보존.
- **보안/프라이버시**: 워크스페이스 스코핑 질의 격리, 적재 시 비밀정보 레드액션 훅, **sync 경계 필터**.
- **노드 신원·전송**: 노드 keypair로 이벤트 **서명**(출처 authenticity), sync는 TLS + 토큰(또는 mTLS) 인증.
- **호스트 정체성**: 안정적 `host_id` 로 출처·동기화 dedup 및 서명 검증의 기준.

---

## 12. 로드맵 (단계)

1. **M0 — 골격 ✅**: workspace 스캐폴드, `core` 모델, in-memory 스토어, `observe`+`get_entity`+`search`(키워드) stdio MCP 서버. (rmcp 0.16, E2E 핸드셰이크 검증 완료)
2. **M1 — 임베디드 스토어**: Cozo 어댑터(관측 로그 + 그래프), `traverse`, Datalog 순회.
3. **M2 — 의미 검색**: `EmbeddingProvider`(fastembed) + HNSW 하이브리드 검색.
4. **M3 — 엔티티 해소 & 스키마**: 해소 규칙, `define_type`, T-Box 검증.
5. **M4 — 페더레이션**: 버전벡터 델타 복제 + sync API. (a) 허브(hub-and-spoke) → (b) 피어(mesh) → (c) 하이브리드. 노드 서명·선택적 공유·충돌/신뢰 정책.
6. **M5 — 추론/추출 포트**: 규칙 기반 경량 추론, 선택적 `Extractor`(LLM/규칙).
...
**확정됨**
- "서버"의 정체(§5): **supragnosis 허브 노드 + 원격 MCP-HTTP 노출** (외부 백엔드 연동은 범위 밖). ✅
- T-Box 부트스트랩: **작은 기본 세트 + 확장** — [`principles.md`](principles.md) 원칙 9로 승격. ✅
...
**현재(M1) 충족**
- 원칙 2(출처 1급): 모든 관측·엔티티·관계가 provenance를 지님.
- 원칙 5(열린 세계): `get_entity` 는 부재를 에러가 아닌 `{found:false, note:…}` 로 반환.
- 원칙 11·18(인코딩 편향 최소·헥사고날): `core` 는 IO 의존 0, Cozo 개념은 `store` 어댑터에만.
  저장소는 `KnowledgeStore` 포트 뒤 — mem/cozo 교체가 도메인 무변경.
- 원칙 13(안정 식별자): 관측 id=blake3 콘텐츠주소, 엔티티 id=정규명 결정적 해소.
- 원칙 17(결정적 코어): 저장·해소·순회 전부 결정론적, 확률적 요소 없음.
- 원칙 19(좁은 표면): 도구 4종(observe/get_entity/search_knowledge/traverse) — 모두 의도 단위.

**의도적 이연 (마일스톤 지정)**
- 원칙 1·6(주장↔믿음 분리, 충돌 보존): 현재 `observe` 는 관측 저장 후 **인라인 단순 프로젝션**.
  관측/관계의 **다중 attestation 누적**과 교체 가능한 해소 정책은 **M3**(해소 계층).
  ※ 진실의 원천인 관측 로그 자체는 보존되므로 재프로젝션으로 회복 가능.
- 원칙 3·4(대체·시간 유효성): supersede/retraction 관측, 관계 valid_from/to, 시간여행 질의 → **M3~M4**.
- 원칙 14(해소는 기반의 책임): 현재는 정규명 완전일치. 임베딩 후보 + 보수적 병합 → **M2~M3**.
- 원칙 15(위상 독립 수렴) + property test: HLC 기반 결정적 재물질화와 무작위 순서 주입
  property test → **M4**(페더레이션).
- 원칙 16(지식 주권): 공유 opt-in 화이트리스트·sync 경계 필터 → **M4**.
- 원칙 20(작업의 부산물): 큐레이션을 질의 결과에 녹이는 UX·프롬프트 → 도구 확장과 함께 점진.
