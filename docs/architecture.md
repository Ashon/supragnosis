...
- 온톨로지 T-Box를 **고정 기본 세트로 부트스트랩** vs **완전 개방형 확장**? (초기: 작은 기본 세트 + 확장)
- 임베딩을 **로컬(fastembed)** vs **클라이언트 공급** vs **원격 API** 중 기본값?
- 1차 페더레이션 위상: **허브(클라이언트-서버)** vs **피어 직접** 중 무엇부터 구현? (설계는 둘 다 수용)
- "서버"의 정체(§5): **supragnosis 허브 노드** vs **원격 MCP-HTTP 노출** vs **외부 지식 백엔드 연동** 중 무엇?
- 충돌 시 "현재 믿음" 정책: **최신 우선** vs **confidence 가중** (또는 둘 다 지원)?
