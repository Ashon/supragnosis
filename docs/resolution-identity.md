...
- **Keyword-search alias parity**: every adapter matches aliases exactly as InMemory does. The
  parity test moved from latent to guard, and then from a check bolted to one backend into a case of
...
| `search_matches_canonical_name_and_alias` | guard (parity) | Section 5 - latent condition retired; a port-wide conformance case, so a new backend inherits it | landed |
